...
MyButton::MyButton(int gpioNumber,int typebouton){
    ...
    isPressed = beginningState; //BeginningState represente the state by default false
    lastRawState = beginningState; 
    waitingSecondClick = beginningState; 
    pendingSingleClick = beginningState; 
    lastDebounceTime = timeBegin; //tiemBegin represent the default value 0
    pressStart = timeBegin; 
    lastReleaseTime = timeBegin; 
    setButtonDebounceDelay(DebounceDelay);
    setButtonLongPressDuration(LongPressDuration);
    setButtonDoubleClickMaxDelay(DoubleClickMaxDelay);
};
